...
{
	printf("%s ", TokenNames[token->tokenType]);
	switch (token->tokenType)
	{
	case TK_COMPARISON:
	case TK_NAME:
		printf("Name: %s @ %d -> %d", (token->value.str), token->loc.startColumn, token->loc.endColumn);
		break;
	case TK_STRING:
		printf("String: %s @ %d -> %d", (token->value.str), token->loc.startColumn, token->loc.endColumn);
		break;
	case TK_KEYWORD:
		printf("Keyword: %s @ %d -> %d", KeywordNames[token->value.number], token->loc.startColumn, token->loc.endColumn);
		break;
	case TK_ASSIGN:
	case TK_TYPE_ASSIGN:
	case TK_SEMICOLON:
	...
	case TK_OPEN_BLOCK:
	case TK_CLOSE_BLOCK:
		printf("%c @ %d -> %d", token->value.c, token->loc.startColumn, token->loc.endColumn);
		break;
	case TK_START:
		printf("<FILE START>");
		break;
...
StLex_Token *StLex_Lex(const char *file)
{
	...
	for (;;)
	{
		...
		newToken->loc.startLine = lexState->currentLine;
		newToken->loc.startColumn = lexState->cursor + 1;
		...
		{ // String
			...
			while (!done)
			{
				if (lexState->current == '"' && escape == 0)
				{
					done = 1;
					break;
				}
				else if (lexState->current == '\\' && escape == 0)
				{
					escape = 2;
				}
				else if (escape == 0)
				{
					...
					str[size] = 0;
					str[size - 1] = lexState->current;
				}
				else if (escape == 1)
				{
					// TODO lookup escape characters
					size++;
					str = realloc(str, sizeof(char) * (size + 1));
					str[size] = 0;
					str[size - 1] = lexState->current;
				...
				// TODO Finish string creation
				nextCharacter(lexState);
			}
			newToken->tokenType = TK_STRING;
			newToken->value.str = str;
		}
		case '\n':
		{
		...
		newToken->loc.endLine = lexState->currentLine;
		newToken->loc.endColumn = lexState->cursor + 1;
